...
int verify_matrix(int16_t *matrix, int16_t *golden_matrix, int64_t R, int64_t C, float threshold) {
    for (int64_t r = 0; r < R; ++r) {
        for (int64_t c = 0; c < C; ++c) {
            float mat_val = (float)matrix[c + C * r];
            float gold_val = (float)golden_matrix[c + C * r];
            
            float abs_diff = fabsf(mat_val - gold_val);
            float rel_diff = abs_diff / (fabsf(gold_val) + 1e-6f); // Add small value to avoid division by zero
            
            if (rel_diff > threshold) {
                printf("Error at o[%lld][%lld]:\n", (long long)r, (long long)c);
                printf("  Computed value: %.9f\n", mat_val);
