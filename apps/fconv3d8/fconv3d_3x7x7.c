...
// Change data types:
// Replace float with int8_t for all input, output, and filter arrays.


// Update vector configuration:
// Change e32 to e8 in all vsetvli instructions.
...
// Replace vle32.v with vle8.v
// Replace vle32.v with vse8.v
...
// Replace floating-point operations with integer operations:
// vfmacc.vf becomes vmacc.vx
// vmul.vx becomes vmul.vx


//or integer operations, we should use the 'r' constraint for general-purpose registers
// eg: asm volatile("vmacc.vx v16, %0, v0" ::"r"(f[0 + base_idx_0]));
...
void fconv3d_CHx7x7_block(int8_t *o, int8_t *i, int8_t *f, int64_t M, int64_t N,
  ...
  // Alternative:
  // int64_t ldo = N * sizeof(int8_t);
  // int64_t ldi_pad = (N + F - 1) * sizeof(int8_t);
  int64_t ldo = N;
